...
# Filter list of domains to process (from info.csv)
...
def processing_func(design_path):
    ...
    # Using a fast pre-filter (like length check) could help, but we want strict TM.
...
def main():
    ...
    # To save time for demonstration, we can process a small logical batch
    # or check if output exists and resume.
    ...
    print("Warning: This will take a LONG time (hours).")
    
    # Batch processing to save progress
    batch_size = 10
    
    with multiprocessing.Pool(NUM_WORKERS, initializer=init_worker, initargs=(refs,)) as pool:
        results = []
